...
delay() & apply_async 返回的都是 AsyncResult 实例，可用于查看任务的执行状态，但首先你要配置好 result backend. 此时，在worker终端上可以看到，任务信息和结果
